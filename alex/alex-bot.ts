...
Objections raised: ${lead.objections_raised || "none recorded"}`;
    ...
    let alexReply = (res.content[0] as { type: string; text: string }).text;

    // Parse and handle DATA line
    const dataMatch = alexReply.match(
      /DATA:name=([^,\n]+),country=([^,\n]+),pain_point=([^,\n]+),email=([^\n]+)/i
    );
    if (dataMatch && !lead.data_collected) {
      await sb
        ...
        .eq("id", lead.id);
      alexReply = alexReply.replace(/DATA:[^\n]*/gi, "").trim();
    }
    ...
    if (
      purchaseWords.some((w) => userText.toLowerCase().includes(w)) &&
    ...
    // Update wind down count and last contacted
